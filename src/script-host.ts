...
export class ScriptHost implements ts.LanguageServiceHost {
  ...
  private readonly _originalFileMap = new Map<string, string | undefined>();
  ...
  updateFile(fileName: string, content: string | undefined) {
    this._fileMap.set(fileName, content);
    if (!this._originalFileMap.has(fileName)) {
      this._originalFileMap.set(fileName, content);
    }
    const currentVersion = this._fileVersionMap.get(fileName) || 0;
    this._fileVersionMap.set(fileName, currentVersion + 1);
  }

  readOriginalFile(fileName: string) {
    const currentVersion = this._fileVersionMap.get(fileName);
    if (!currentVersion) return;
    return this._originalFileMap.get(fileName);
  }
}
